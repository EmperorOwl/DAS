""" Code for the recording of the bot's usage. """
import datetime
import discord
from discord.ext import commands, tasks

from src.config import STATS_CHANNEL, STATS_FILE
...
class Stats(commands.Cog):
    ...
    def _get_stats_embed(self, stats: dict) -> discord.Embed:
        ...
        for time_period in stats:
            ...
            graph_cmds_per = int(graph_cmds / graphs *
                                 100) if graphs != 0 else 0
            graph_btns_per = int(graph_btns / graphs *
                                 100) if graphs != 0 else 0
            # Compute average
            if time_period == 'all_time' and embed.description is not None:
    ...
    async def stats(self, ctx: commands.Context) -> None:
        ...
        stats = read_data_from_json_file(STATS_FILE)
        if not stats:
    ...
    async def on_interaction(self, itx: discord.Interaction):
        ...
        write_data_to_json_file(STATS_FILE, stats)
        ...
        if stats:
            if STATS_CHANNEL:
                ...
                if isinstance(channel, discord.TextChannel):
                    await channel.send(embed=self._get_stats_embed(stats))
